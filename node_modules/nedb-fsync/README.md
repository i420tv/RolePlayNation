...
* `flushDataOnCompaction` (optional): explicitly uses fsync when doing auto compaction. For example, ext4 file system utilized delayed allocation which means that when data is written to a file, it is not persisted until after a delay. That can cause severe data loss if the system crashes and the data has not yet been flushed. Using this option has some performance hit, but on the other hand it only affects the auto compaction which usually happens quite infrequently.
