...
namespace WiredPlayers.globals
{
    public class Globals : Script
    {
        ...
        public static List<TruckingOrderModel> truckingOrderList;
        // public static List<BurgerFlopperOrderModel> WeedsmokeList;
        ...
        private static Timer Weedsmoke;
        private static Timer Cocaine;
        public static Dictionary<int, Timer> WeedsmokeList;
        public static Dictionary<int, Timer> CocaineList;
        ...
        public Globals()
        {
            WeedsmokeList = new Dictionary<int, Timer>();
            CocaineList = new Dictionary<int, Timer>();
            ...
            truckingOrderList = new List<TruckingOrderModel>();
        ...
        private void OnMinuteSpent(object unused)
        {
            NAPI.Task.Run(() =>
            {
               ...
               */
                ...
                {
                    Random rnd = new Random();
                    int generatedOrders = rnd.Next(5, 20);
                    for (int i = 0; i < generatedOrders; i++)
                    {
                ...
                // Generate new Trucking orders
                if (orderGenerationTime <= totalSeconds && Business.businessList.Count > 0)
                {
                    Random rnd = new Random();
                    int generatedOrders = rnd.Next(20, 50);
                    for (int i = 0; i < generatedOrders; i++)
                    {
                        TruckingOrderModel order = new TruckingOrderModel();
                        {
                            order.id = fastFoodId;
                            order.pizzas = rnd.Next(0, 4);
                            order.hamburgers = rnd.Next(0, 4);
                            order.sandwitches = rnd.Next(0, 4);
                            order.position = GetPlayerTruckingDeliveryDestination();
                            order.limit = totalSeconds + 300;
                            order.taken = false;
                        }

                        truckingOrderList.Add(order);
                        fastFoodId++;
                    }
                ...
                // Remove old orders
                truckingOrderList.RemoveAll(order => !order.taken && order.limit <= totalSeconds);
        ...
        private Vector3 GetPlayerTruckingDeliveryDestination()
        {
            Random random = new Random();
            int element = random.Next(Business.businessList.Count);
            return Business.businessList[element].position;
        }
        ...
        private void ConsumeItem(Client player, ItemModel item, BusinessItemModel businessItem, bool consumedFromHand)
        {
            ...
            if (businessItem.armor != 0)
            {
                player.Armor += businessItem.armor;
        ...
        [RemoteEvent("teststats")]

        public void statsui(Client player)
        {
        ...
        /*  public static List<InventoryModel> GetHouseInventory(House house)
          {

              List<InventoryModel> inventory = new List<InventoryModel>();
              int houseId = House.GetHouseById;
              foreach (ItemModel item in itemList)
              {
                  if (item.ownerEntity == Constants.ITEM_ENTITY_HOUSE && item.ownerIdentifier == houseId)
                  {
                      // Check whether is a common item or a weapon
                      InventoryModel inventoryItem = new InventoryModel();
                      BusinessItemModel businessItem = Business.GetBusinessItemFromHash(item.hash);
        ...
        public void OnPlayerDisconnected(Client player, DisconnectionType type, string reason)
        {
            ...
            if (player.GetData(EntityData.PLAYER_PLAYING) != null)
            {
                ...
                Chat.SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_DISCONNECT, 10.0f);
            }
        }
                ...
                if (lobbyExit.DistanceTo(player.Position) > 1.75f)
                {
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        // Welcome message
                        player.SendChatMessage(Constants.COLOR_WHITE + "Welcome to " + Constants.COLOR_OCEANBLUE + "Vital Roleplay. " + Constants.COLOR_WHITE + "We hope you enjoy your time here!");
                        player.SendChatMessage(Constants.COLOR_WHITE + "To check your profile, use " + Constants.COLOR_YELLOW + "F4");

                        player.SendChatMessage(Constants.COLOR_WHITE + "If you need help, you can use " + Constants.COLOR_YELLOW + "/help");
        ...
        private static void CokeDowner(Client player, bool isMining)
        {
            if (isMining)
            {
                // Create the mining timer
                Weedsmoke = new Timer(RemoveBuff, player, 18000, Timeout.Infinite);
            }
            // Add the timer to the list
            CocaineList.Add(player.Value, Weedsmoke);
        }
        private static void RemoveBuff(object Weed)
        {
            Client player = (Client)Weed;
            if (player.Armor < 24)
            {
                player.Health -= 25;
            }
            ...
            CocaineList.Remove(player.Value);
            NAPI.ClientEvent.TriggerClientEvent(player, "DisableScreenFx");

        }



        private static void CollectBuff(object Weed)
        {
            Client player = (Client)Weed;
            player.Health += 10;
            if (player.Health > 100) player.Health = 100;
            player.StopAnimation();
            player.SetData(EntityData.PlayerMining, false);
            WeedsmokeList.Remove(player.Value);

        }
        private static void CollectBuffCoke(object Weed)
        {
            Client player = (Client)Weed;
            player.Armor += 25;
            if (player.Armor > 50) player.Armor = 50;
            player.StopAnimation();
            player.SetData(EntityData.PlayerMining, false);
            CocaineList.Remove(player.Value);
            CokeDowner(player, true);

        }
        public static void AnimateSmokingWeed(Client player, bool isMining)
        {
            if(WeedsmokeList.ContainsKey(player.Value))
            {
                player.SendChatMessage(Constants.COLOR_ERROR + "You are already smoking weed");
                return;
            }
            // Play the animation
            player.PlayAnimation("amb@world_human_aa_smoke@male@idle_a", "idle_a", (int)Constants.AnimationFlags.Loop | (int)(Constants.AnimationFlags.AllowPlayerControl));
            player.SetSharedData(EntityData.PLAYER_WALKING_STYLE, "move_m@drunk@verydrunk");
            NAPI.ClientEvent.TriggerClientEvent(player, "EnableWeedFx");
            NAPI.ClientEvent.TriggerClientEventForAll("changePlayerWalkingStyle", player.Handle, "move_m@drunk@verydrunk");
            // Create the timer
            Timer Weedsmoke = null;

            if (isMining)
            {
                // Create the mining timer
                Weedsmoke = new Timer(CollectBuff, player, 20000, Timeout.Infinite);
            }
            // Add the timer to the list
            WeedsmokeList.Add(player.Value, Weedsmoke);
        }
        public static void AnimateCocaine(Client player, bool isMining)
        {
            if(CocaineList.ContainsKey(player.Value))
            {
                player.SendChatMessage(Constants.COLOR_ERROR + "You are already taking a line of cocaine");
                return;
            }
            // Play the animation
            player.PlayAnimation("missfbi3_party", "snort_coke_b_male3", (int)Constants.AnimationFlags.Loop | (int)(Constants.AnimationFlags.AllowPlayerControl));
            NAPI.ClientEvent.TriggerClientEvent(player, "EnableScreenFx");
            // Create the timer
            Timer Cocaine = null;
            ...
            {
                // Create the mining timer
                Cocaine = new Timer(CollectBuffCoke, player, 20000, Timeout.Infinite);
            }
            // Add the timer to the list
            CocaineList.Add(player.Value, Cocaine);
        ...
        public void ProcessMenuActionEvent(Client player, int itemId, string action)
        {
            ...
            switch (action.ToLower())
            {
                case Commands.COM_CONSUME:
                    ...
                    if (item.hash == Constants.ITEM_HASH_WEED)
                    {
                        AnimateSmokingWeed(player, true);
                    };
                    if (item.hash == Constants.ITEM_HASH_COCAIN)
                    {
                        AnimateCocaine(player, true);
                    }
                ...
                case Commands.ARG_WITHDRAW:
                    ...
                    Chat.SendMessageToNearbyPlayers(timeString, player, InfoRes.trunk_item_withdraw, Constants.MESSAGE_ME, 20.0f);
        ...
        private const int MaxRoll = 10;
        [Command("roll")]
        public void RollCommand(Client player)
        {
            string timeString = "[" + DateTime.Now.ToString("HH:mm");
            Random random = new Random();
            int amount = random.Next(1, MaxRoll);
            Chat.SendMessageToNearbyPlayers(timeString, player, "Rolled "+ amount, 3, 5f);
        }
        [RemoteEvent("actionkeyE")]
        public void OpenUi(Client player)
        {
            Vector3 OceantriggerPosition = new Vector3(312.4634f, -2961.938f, 6.962742f);
            Vector3 TruckertriggerPosition = new Vector3(-44.04705f, -2519.901f, 7.394626f);
            Vector3 PostaltriggerPosition = new Vector3(499.7191f, -651.9603f, 24.90868f);
            Vector3 FastfoodtriggerPosition = new Vector3(177.1226f, -1437.733f, 29.24163f);
            Vector3 HookertriggerPosition = new Vector3(105.7209f, -1302.901f, 28.7688f);
            Vector3 MechanictriggerPosition = new Vector3(119.923f, 6627.261f, 31.94834f);
            Vector3 TownhalltriggerPosition = new Vector3(-139.2177f, -631.8386f, 168.86f);
            Vector3 SeedDealertriggerPosition = new Vector3(1301.98f, 4229.977f, 33.90854f);
            Vector3 DrugDealertriggerPosition = new Vector3(1257.495f, -2571.437f, 42.71827f);
            Vector3 Blackmarket1 = new Vector3(-581.2188f, -1612.64f, 27.01082f);
            Vector3 Blackmarket2 = new Vector3(1364.366f, -2102.625f, 51.99857f);
            foreach (PlantModel plant in Plants)
            {
                if (plant.Progress.Position.DistanceTo(player.Position) < 5)
                {
                    Drugs.HarvestweedCommand(player);
                }
            }
            if (OceantriggerPosition.DistanceTo(player.Position) < 10.0f)
            {
                // Show the Job menu
                player.TriggerEvent("showOceanCleanerUi");
            }
            if (TruckertriggerPosition.DistanceTo(player.Position) < 5f)
            {
                player.TriggerEvent("showTruckerUi");
            }
            if (PostaltriggerPosition.DistanceTo(player.Position) < 5f)
            {
                // Show the Job menu
                player.TriggerEvent("showPostalUi");
            }
            if (FastfoodtriggerPosition.DistanceTo(player.Position) < 5f)
            {
                player.TriggerEvent("showFastfoodUi");
            }
            if (HookertriggerPosition.DistanceTo(player.Position) < 5f)
            {
                // Show the Job menu
                player.TriggerEvent("showPostalUi");
            }
            if (MechanictriggerPosition.DistanceTo(player.Position) < 5f)
            {
                player.TriggerEvent("showFastfoodUi");
            }
            if (TownhalltriggerPosition.DistanceTo(player.Position) < 5f)
            {
                player.TriggerEvent("showTownHallMenu");
            }
            if (SeedDealertriggerPosition.DistanceTo(player.Position) < 5f)
            {
                player.TriggerEvent("showFastfoodUi");
            }
            if (DrugDealertriggerPosition.DistanceTo(player.Position) < 5f)
            {
                player.TriggerEvent("showTownHallMenu");
            }
            if (player.GetData(EntityData.PLAYER_BUSINESS_ENTERED) > 0)
            {
                ...
                switch (business.type)
                {
                    ...
                    case Constants.BUSINESS_TYPE_BARBER_SHOP:
                        ...
                        break;
                    case Constants.BUSINESS_TYPE_BLACK_MARKET:
                        // Temporary
                        if (Blackmarket1.DistanceTo(player.Position) < 5f)
                        {
                        ...
                        if(Blackmarket2.DistanceTo(player.Position) < 5f)
                        {
                            // Show the purchase menu
                            player.TriggerEvent("showBusinessPurchaseMenu", NAPI.Util.ToJson(businessItems), business.name, business.multiplier);
                        }
            ...
            {
                // Get all the Copper nodes
                foreach (OreModel CopperItem in Mining.CopperList)
                {
                    if (player.Position.DistanceTo(CopperItem.itemHud.Position) <= 1.5f)
                    {
                        Mining.MiningCommand(player);
                        return;
                    }
                }
                foreach (OreModel IronItem in Mining.IronList)
                {
                    if (player.Position.DistanceTo(IronItem.itemHud.Position) <= 1.5f)
                    {
                        Mining.IronCommand(player);
                        return;
                    }
                }
                foreach (OreModel TinItem in Mining.TinList)
                {
                    if (player.Position.DistanceTo(TinItem.itemHud.Position) <= 1.5f)
                    {
                        Mining.TinCommand(player);
                        return;
                    }
                }
                foreach (OreModel GoldItem in Mining.GoldList)
                {
                    if (player.Position.DistanceTo(GoldItem.itemHud.Position) <= 1.5f)
                    {
                        Mining.GoldCommand(player);
                        return;
                    }
                }
                foreach (OreModel LeadItem in Mining.LeadList)
                {
                    if (player.Position.DistanceTo(LeadItem.itemHud.Position) <= 1.5f)
                    {
                        Mining.LeadCommand(player);
                        return;
                    }
                }
                foreach (OreModel SodiumItem in Mining.SodiumList)
                {
                    if (player.Position.DistanceTo(SodiumItem.itemHud.Position) <= 1.5f)
                    {
                        Mining.SodiumCommand(player);
                        return;
                    }
                }
                foreach (PlantModel plant in Drugs.Plants)
                {
                    if (player.Position.DistanceTo(plant.Position) <= 1.5f)
                    {
                        Drugs.HarvestweedCommand(player);
                        return;
                    }
                ...
                {
                    if(player.Position.DistanceTo(oceanItem.itemPosition) < 10)
                    {
                        OceanCleaner.Command_Job_OceanCleaner_Collect(player);
                        return;
                    }
                }

                // Get all the Leafs
                foreach (PlantModel LeafItem in Drugs.allCocainItems)
                {
                    if (player.Position.DistanceTo(LeafItem.itemHud.Position) <= 1.5f)
                    {
                        Drugs.HarvestCommand(player);
                        return;
                    }
                }
                foreach (OreModel Box in Bankheist.BoxList)
                {
                    if (player.Position.DistanceTo(Box.itemHud.Position) <= 1.5f)
                    {
                        Bankheist.DrillBoxes(player);
                        return;
                    }
                }
                foreach (Vector3 terminal in Constants.HACKABLE_TERMINALS)
                {
                    ...
                    {
                        Bankheist.HackTerminal(player);
                        return;
                    }
                }
                // Get all the Copper nodes
                if (player.Position.DistanceTo(new Vector3(1112.388f, -2005.329f, 35.4394f)) <= 1.5f)
                {
                    Mining.SmeltCommand(player);
                    return;
                }
            }
            if (player.GetData(EntityData.PLAYER_PLAYING) != null)
        ...
        {
            player.SendChatMessage(Constants.COLOR_ERROR + "For help Go to #i_have_a_question in discord");
        ...
        {
            player.SendChatMessage(Constants.COLOR_ERROR + "~w~Welcome to ~o~Vital~w~ Roleplay!");
            ...
            else
            {
                ...
                switch (documentation.ToLower())
                {
                    case Commands.ARG_LICENSES:
                        ...
                        Chat.SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_ME, 20.0f);
        ...
        public void CancelCommand(Client player, string cancel)
        {
            switch (cancel.ToLower())
            {
                ...
                case Commands.ARG_ORDER:
                    ...
                    break;
                case Commands.ARG_ATTENDANCE:
                    if (player.GetData(EntityData.PLAYER_FACTION_WARNING) != null)
                    {
                        player.ResetData(EntityData.PLAYER_FACTION_WARNING);

                        player.SendChatMessage(Constants.COLOR_INFO + "You canceled the Attendance for the report.");
                    }
                    break;
        ...
        public void Lock(Client player)
        {
        ...
        public void LockCommand(Client player)
        {
            ...
            else
            {
                ...
                //}
                    ...
                    Chat.SendMessageToNearbyPlayers(timeString, player, vehicle.Locked ? SuccRes.veh_locked : SuccRes.veh_unlocked, Constants.MESSAGE_ME, 20.0f);
