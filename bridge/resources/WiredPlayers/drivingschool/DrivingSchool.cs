...
namespace WiredPlayers.drivingschool
{
    public class DrivingSchool : Script
    {
        ...
        private void FinishDrivingExam(Client player, Vehicle vehicle)
        {
            ...
            {
                if (v.owner == playerName && v.model == (uint)(VehicleHash.Washington))
                {
                    int id = v.id;
            ...
            foreach (VehicleModel v in vehicles)
            {
                if (v.owner == playerName && v.model == (uint)(VehicleHash.BF400))
                {
                    int id = v.id;
                    veh = Vehicles.GetVehicleById(id);
                    int vehicleId = veh.GetData(EntityData.VEHICLE_ID);
            ...
            foreach (VehicleModel v in vehicles)
            {
                if (v.owner == playerName && v.model == (uint)(VehicleHash.Packer))
                {
                    int id = v.id;
                    veh = Vehicles.GetVehicleById(id);
                    int vehicleId = veh.GetData(EntityData.VEHICLE_ID);
                ...
                if (interior.captionMessage == GenRes.driving_school && player.Position.DistanceTo(interior.entrancePosition) < 2.5f)
                {
                    ...
                    switch (type.ToLower())
                    {
                            ...
                            switch (licenseStatus)
                            {
                                ...
                                case 0:
                                    ...
                                    if (money >= Constants.PRICE_DRIVING_PRACTICAL)
                                    {
                                        ...
                                        if (player.GetData(EntityData.PLAYER_DRIVING_EXAM) == 2)
                                        {
                                            // Basic data for vehicle creation
                                            vehicle.model = (uint)VehicleHash.Washington;
                                            vehicle.faction = Constants.FACTION_DRIVING_SCHOOL;
                                            vehicle.position = new Vector3(-908.1901f, -2044.644f, 9.299003f);
                        ...
                        case Commands.ARG_MOTORCYCLE:
                            ...
                            switch (licenseStatus)
                            {
                                ...
                                case 0:
                                    ...
                                    if (money >= Constants.PRICE_DRIVING_PRACTICAL)
                                    {
                                        ...
                                        VehicleModel vehicle = new VehicleModel();
                                        // Basic data for vehicle creation
                                        vehicle.model = (uint)VehicleHash.BF400;
                                        vehicle.faction = Constants.FACTION_DRIVING_SCHOOL;
                                        vehicle.position = new Vector3(-905.5831, -2042.533, 9.299003f);
                                        vehicle.rotation = player.Rotation;
                                        vehicle.dimension = player.Dimension;
                                        vehicle.colorType = Constants.VEHICLE_COLOR_TYPE_CUSTOM;
                                        vehicle.firstColor = "256,256,256";
                            ...
                            break;
                        case Commands.ARG_TRUCKING:
                            // Check for the status if the license
                            licenseStatus = GetPlayerLicenseStatus(player, Constants.LICENSE_TRUCKER);
                            ...
                            switch (licenseStatus)
                            {
                                case -1:
                                    ...
                                    if (money >= Constants.PRICE_DRIVING_THEORICAL)
                                    {
                                        Task.Factory.StartNew(() =>
                                        {
                                            ...
                                            {
                                                // Add the questions
                                                questions = Database.GetRandomQuestions(Constants.LICENSE_TRUCKER + 1, Constants.MAX_LICENSE_QUESTIONS);
                                                foreach (TestModel question in questions)
                                                {
                                                    answers.AddRange(Database.GetQuestionAnswers(question.id));
                                                }

                                                player.SetData(EntityData.PLAYER_LICENSE_TYPE, Constants.LICENSE_TRUCKER);
                                                player.SetSharedData(EntityData.PLAYER_LICENSE_QUESTION, 0);
                                ...
                                case 0:
                                    ...
                                    if (money >= Constants.PRICE_DRIVING_PRACTICAL)
                                    {
                                        ...
                                        player.SendChatMessage(Constants.COLOR_INFO + "Your truck is parked outside, go ahead and drive around the track.");
                                        int vehicleId = 0;
                                        VehicleModel vehicle = new VehicleModel();
                                        // Basic data for vehicle creation
                                        vehicle.model = (uint)VehicleHash.Packer;
                                        vehicle.faction = Constants.FACTION_DRIVING_SCHOOL;
                                        vehicle.position = new Vector3(-887.9573, -2039.36, 9.299141f);
                                        vehicle.rotation = player.Rotation;
                                        vehicle.dimension = player.Dimension;
                                        vehicle.colorType = Constants.VEHICLE_COLOR_TYPE_CUSTOM;
                                        vehicle.firstColor = "256,256,256";
            ...
            foreach (string license in playerLicensesArray)
            {
                ...
                switch (currentLicense)
                {
                    case Constants.LICENSE_CAR:
                        ...
                        break;
                    case Constants.LICENSE_TRUCKER:
                        switch (currentLicenseStatus)
                        {
                            case -1:
                                player.SendChatMessage(Constants.COLOR_HELP + InfoRes.car_license_not_available);
