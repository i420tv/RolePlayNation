...
namespace WiredPlayers.house
{
    public class House : Script
    {
        ...
        public static void BuyHouse(Client player, HouseModel house)
        {
           ...
           */
            if (player.GetData(EntityData.PLAYER_HOUSES) > 2)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + "You already have three houses. You'll need to speak to the goverment to get more. " + Constants.COLOR_YELLOW + "/premium");
                return;
            }
