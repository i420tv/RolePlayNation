...
namespace WiredPlayers.character
{
    public class Animations : Script
    {
        ...
        [Command(Commands.COM_INVESTIGATE)]
        public void InvestigateCommand(Client player)
        {
            if (player.GetSharedData(EntityData.PLAYER_KILLED) != 0)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + ErrRes.player_is_dead);
            ...
            {
                player.StopAnimation();
                player.PlayAnimation("amb@code_human_police_investigate@base", "base", (int)(Constants.AnimationFlags.Loop | Constants.AnimationFlags.AllowPlayerControl));
            }

        }
        [Command(Commands.COM_CROWDCONTROL)]
        public void CrowdCommand(Client player)
        {
            if (player.GetSharedData(EntityData.PLAYER_KILLED) != 0)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + ErrRes.player_is_dead);
            ...
            {
                player.StopAnimation();
                player.PlayAnimation("amb@code_human_police_crowd_control@base", "base", (int)(Constants.AnimationFlags.Loop | Constants.AnimationFlags.AllowPlayerControl));
            }
        }
      /*  [Command(Commands.COM_CROWDCONTROL)]
        public void PaparaziCommand(Client player)
        {
            if (player.GetSharedData(EntityData.PLAYER_KILLED) != 0)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + ErrRes.player_is_dead);
            ...
            {
                player.StopAnimation();
                player.PlayAnimation("amb@world_human_paparazzi@male@base", "base", (int)(Constants.AnimationFlags.Loop | Constants.AnimationFlags.AllowPlayerControl));
            }
        }
        [Command(Commands.COM_BARBELL)]
        public void BarbellCommand(Client player)
        {
            if (player.GetSharedData(EntityData.PLAYER_KILLED) != 0)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + ErrRes.player_is_dead);
            ...
            {
                player.StopAnimation();
                player.PlayAnimation("amb @world_human_muscle_free_weights@male @barbell@base", "base", (int)(Constants.AnimationFlags.Loop | Constants.AnimationFlags.AllowPlayerControl));
            }
        }*/
    }
