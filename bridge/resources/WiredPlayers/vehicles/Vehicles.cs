...
namespace WiredPlayers.vehicles
{
    public class Vehicles : Script
    {
        ...
        public static Vehicle CreateIngameVehicle(VehicleModel vehModel)
        {
            ...
            Lsc.AddTunningToVehicle(vehicle);
        ...
        public void OnPlayerEnterVehicle(Client player, Vehicle vehicle, sbyte seat)
        {
            ...
            if (Convert.ToInt32(seat) == (int)VehicleSeat.Driver)
            {
                ...
                if (vehicle.Model == (uint)VehicleHash.Prototipo)
                {
                ...
                player.CurrentWeapon.Equals("none");
            }
        }
        ...
        {
            // Your fuel is at 5
            player.SendChatMessage(Constants.COLOR_ERROR + "Your car seems to be low on gas.");
        }
        ...
        public void EngineOnEventKeyEvent(Client player)
        {
            ...
            else if (player.Vehicle.Health < 500)
            {
                Enginetrouble(player, true);
            }
            else
            {
                player.Vehicle.EngineStatus = true;
        ...
        public void Enginetrouble(Client player, bool isCooking)
        {
            ...
            player.PlayAnimation("amb@prop_human_parking_meter@male@idle_a", "idle_a", (int)(Constants.AnimationFlags.AllowPlayerControl | Constants.AnimationFlags.Loop));
                ...
                if (isCooking)
                {
                    ...
                    miningTimer = new Timer(StartVehicle, player, 12000, Timeout.Infinite);
                }
                // Add the timer to the list
                harvestTimerList.Add(player.Value, miningTimer);
        
        }
        public void StartVehicle(object Engine)
        {
            Client player = (Client)Engine;
        ...
        public void ToggleSeatbeltEvent(Client player, bool seatbelt)
        {
            ...
            Chat.SendMessageToNearbyPlayers(timeString, player, seatbelt ? InfoRes.seatbelt_fasten : InfoRes.seatbelt_unfasten, Constants.MESSAGE_ME, 20.0f);
        }
        ...
        [RemoteEvent("OpenDoors")]
        public void OpenDoors(Client player)
        {
        ...
        public void opentrunkinventory(Client player)
        {
            Vehicle vehicle = GetClosestVehicle(player, 3.75f);
        ...
        public void closetrunk(Client player)
        {
            Vehicle vehicle = GetClosestVehicle(player, 3.75f);
            List<bool> doorState = NAPI.Util.FromJson<List<bool>>(vehicle.GetSharedData(EntityData.VEHICLE_DOORS_STATE));
            if (!HasPlayerVehicleKeys(player, vehicle, false))
