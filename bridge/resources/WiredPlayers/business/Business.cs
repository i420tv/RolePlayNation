...
namespace WiredPlayers.business
{
    public class Business : Script
    {
        ...
        public static void LoadDatabaseBusiness()
        {
            ...
            foreach (BusinessModel businessModel in businessList)
            {
                if(businessModel.ipl.Length == 0)
                {
                    ...
                    businessModel.businessColshape = NAPI.ColShape.CreateCylinderColShape(businessModel.position, 2.5f, 1.0f);
                    businessModel.businessMarker = NAPI.Marker.CreateMarker(MarkerType.HorizontalCircleSkinny, businessModel.position, new Vector3(), new Vector3(), 2.5f, new Color(198, 40, 40, 200));
                    Vector3 businessPos = businessModel.position;
                    Vector3 newPosition = businessPos;              

                    if (businessModel.type == 12)
                    {
                        businessModel.businessLabel = NAPI.TextLabel.CreateTextLabel("Gas Pump", newPosition , 5.0f, 2.0f, 4, new Color(255, 255, 255), false, 0); // Business Name
                        businessModel.businessLabel = NAPI.TextLabel.CreateTextLabel("/refuel", new Vector3(newPosition.X, newPosition.Y, newPosition.Z - 0.2f), 5.0f, 0.75f, 4, new Color(255, 255, 255), false, 0); // Business Command Info
                    }
                    else
                        businessModel.businessLabel = NAPI.TextLabel.CreateTextLabel(businessModel.name, newPosition, 5.0f, 0.75f, 4, new Color(255, 255, 255), false, 0);
        ...
        public static void BuyBusiness(Client player, BusinessModel business)
        {
           // if (player.GetData(EntityData.PLAYER_LEVEL) < 10)
          //  {
          //      player.SendChatMessage(Constants.COLOR_ERROR + "You need to be Level 10 in order to purchase your first business.");
           //     return;
          //  }

            if (player.GetData(EntityData.PLAYER_BUSINESSES) > 0)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + "You already have one Businesses. You'll need to speak to the goverment to get more. " + Constants.COLOR_YELLOW + "/premium");
                return;
            }

            if (business.status != Constants.BUSINESS_STATE_BUYABLE)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + ErrRes.business_not_buyable);
                return;
            }

            if (player.GetSharedData(EntityData.PLAYER_BANK) < business.price)
            {
                player.SendChatMessage(Constants.COLOR_ERROR + ErrRes.business_not_money);
                return;
            }

            int bank = player.GetSharedData(EntityData.PLAYER_BANK) - business.price;
            string message = string.Format(InfoRes.business_buy, business.name, business.price);
            player.SendChatMessage(Constants.COLOR_INFO + message);
            player.SetSharedData(EntityData.PLAYER_BANK, bank);
            business.status = Constants.BUSINESS_STATE_NONE;
        ...
        public void PlayerEnterColshapeEvent(ColShape colShape, Client player)
        {
            ...
            BusinessModel business = businessList.Where(b => player.Position.DistanceTo(b.position) <= 5f).FirstOrDefault();
                    ...
                    Task.Factory.StartNew(() => 
                    {
                        NAPI.Task.Run(() =>
                        {
                            ...
                            if (itemModel == null || businessItem.hash == Constants.ITEM_HASH_TELEPHONE)
                            {
                                ...
                                {
                                    itemModel.ownerEntity = Constants.ITEM_ENTITY_PLAYER; //int.TryParse(itemModel.hash, out hash) ?/* Constants.ITEM_ENTITY_RIGHT_HAND :*/ Constants.ITEM_ENTITY_PLAYER;
                                }
