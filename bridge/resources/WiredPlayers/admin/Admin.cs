...
namespace WiredPlayers.admin
{
    public class Admin : Script
    {
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) == 8)
            {
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_MODERATOR)
            {
                player.Dimension = 0;
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_ADMIN)
            {
                // We get the player from the input string
                Client target = int.TryParse(targetString, out int targetId) ? Globals.GetPlayerById(targetId) : NAPI.Player.GetPlayerFromName(targetString);
        ...
        {
            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
            {
                int vehicleId = 0;
                Vehicle veh = null;
                ...
                if (args.Trim().Length > 0)
                {
                    ...
                    switch (arguments[0].ToLower())
                    {
                        ...
                        case Commands.ARG_BRING:
                            if (player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_SUPPORT)
                            {
                                if (arguments.Length == 2 && int.TryParse(arguments[1], out vehicleId) == true)
                                {
                                    veh = Vehicles.GetVehicleById(vehicleId);
        ...
        {
            if (HasUserCommandPermission(player, Commands.COM_BUSINESS) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
            {
                if (args.Trim().Length > 0)
                {
                    ...
                    switch (arguments[0].ToLower())
                    {
                        case Commands.ARG_INFO:
                            break;
                        case Commands.ARG_CREATE:
                            if (HasUserCommandPermission(player, Commands.COM_BUSINESS, Commands.ARG_CREATE) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                            {
                                if (arguments.Length == 3)
                                {
                                            ...
                                            NAPI.Task.Run(() =>
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    if (type == 1)
                                                    {
                                                        ...
                                                        business.businessLabel = NAPI.TextLabel.CreateTextLabel("Press E to open", new Vector3(newPosition.X, newPosition.Y, newPosition.Z - 0.2f), 5.0f, 0.75f, 4, new Color(255, 255, 255), false, 0); // Business Command Info    
                                                    }
                                                    if (type == 4)
                                                    {
                                                    ...
                                                    else
                                                    {
                                                       ...
                                                       // business.businessColshape = NAPI.ColShape.CreateCylinderColShape(business.position, 2.5f, 5.0f);
                                                        business.businessMarker = NAPI.Marker.CreateMarker(MarkerType.VerticalCylinder, business.position, new Vector3(), new Vector3(), 2.0f, new Color(198, 40, 40, 200));
                                                    }
                                                }
                            ...
                            break;
                        case Commands.ARG_MODIFY:
                            if (HasUserCommandPermission(player, Commands.COM_BUSINESS, Commands.ARG_MODIFY) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                            {
        ...
        public void HouseCommand(Client player, string args)
        {
            ...
            switch (arguments[0].ToLower())
            {
                case Commands.ARG_INFO:
                    ...
                    break;
                case Commands.ARG_CREATE:
                    if (HasUserCommandPermission(player, Commands.COM_HOUSE, Commands.ARG_CREATE) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                    {
                ...
                case Commands.ARG_MODIFY:
                    if (arguments.Length > 2)
                    {
                        ...
                        if (int.TryParse(arguments[2], out int value) == true)
                        {
                            ...
                            switch (arguments[1].ToLower())
                            {
                                case Commands.ARG_INTERIOR:
                                    if (HasUserCommandPermission(player, Commands.COM_HOUSE, Commands.ARG_INTERIOR) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                                    {
                                ...
                                case Commands.ARG_PRICE:
                                    if (HasUserCommandPermission(player, Commands.COM_HOUSE, Commands.ARG_PRICE) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                                    {
                                        if (value > 0)
                                        {
                                            house.price = value;
                        ...
                        else
                        {
                            ...
                            {
                                case Commands.ARG_OWNER:
                                    if (HasUserCommandPermission(player, Commands.COM_HOUSE, Commands.ARG_OWNER) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                                    {
                                    ...
                                    break;
                                case Commands.ARG_NAME:
                                    if (HasUserCommandPermission(player, Commands.COM_HOUSE, Commands.ARG_NAME) || player.GetData(EntityData.PLAYER_ADMIN_RANK) > Constants.STAFF_SENIOR_MODERATOR)
                                    {
