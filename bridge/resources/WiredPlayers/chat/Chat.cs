...
namespace WiredPlayers.chat
{
    public class Chat : Script
    {
        ...
        public static void SendMessageToNearbyPlayers(string Time, Client player, string message, int type, float range, bool excludePlayer = false)
        {
                    ...
                    if (distance <= range)
                    {
                        ...
                        switch (type)
                        {
                            ...
                            case Constants.MESSAGE_TALK:
                                target.SendChatMessage(chatMessageColor + timeString + fakeName + GenRes.chat_say + message);
                                break;
                            case Constants.MESSAGE_SHOUT:
                                target.SendChatMessage(chatMessageColor + timeString + fakeName + GenRes.chat_yell + message + "!");
                                break;
                            case Constants.MESSAGE_WHISPER:
                                target.SendChatMessage(chatMessageColor + timeString + fakeName + GenRes.chat_whisper + message);
                                break;
                            case Constants.MESSAGE_PHONE:
                                target.SendChatMessage(chatMessageColor + timeString + fakeName + GenRes.chat_phone + message);
                                break;
                            case Constants.MESSAGE_RADIO:
                                target.SendChatMessage(chatMessageColor + timeString + fakeName + GenRes.chat_radio + message);
                                break;
                            case Constants.MESSAGE_ME:
                                target.SendChatMessage(Constants.COLOR_CHAT_ME + timeString + fakeName + " " + message);
                                break;
                            case Constants.MESSAGE_DO:
                                target.SendChatMessage(Constants.COLOR_CHAT_DO + timeString + fakeName + "[ID: " + player.Value + "] " + message);
                                break;
                            case Constants.MESSAGE_OOC:
                                target.SendChatMessage(oocMessageColor + timeString + "(([ID: " + player.Value + "] " + fakeName + ": " + message + "))");
                                break;
                            case Constants.MESSAGE_DISCONNECT:
                                target.SendChatMessage(Constants.COLOR_HELP + timeString + "[ID: " + player.Value + "] " + fakeName + ": " + message);
                                break;
                            case Constants.MESSAGE_MEGAPHONE:
                                target.SendChatMessage(Constants.COLOR_INFO + timeString + "[Megáfono de " + fakeName + "]: " + message);
                                break;
       ...
       */
        ...
        public void OnChatMessage(Client player, string message)
        {
            ...
            else if (player.GetData(EntityData.PLAYER_PHONE_TALKING) != null)
            {
                ...
                SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_PHONE, player.Dimension > 0 ? 7.5f : 10.0f, true);
            }
            else
            {
                // We send the message to nearby players
                ...
                SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_TALK, player.Dimension > 0 ? 7.5f : 10.0f, true);
            }

            // Log the message on the server           
            string timerString = "[" + DateTime.Now.ToString("HH:mm:ss") + "." + DateTime.Now.Millisecond + "] ";
            NAPI.Util.ConsoleOutput(timerString + "[ID:" + player.Value + "] " + player.Name + GenRes.chat_say + message);
        }
        ...
        public void CheckPlayerStats(Client player)
        {
            ...
            NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_SANDYORANGE + "[Name: " + Constants.COLOR_WHITE + player.GetData(EntityData.PLAYER_NAME) + Constants.COLOR_SANDYORANGE + " ( " + Constants.COLOR_WHITE + "Player: " + player.GetData(EntityData.PLAYER_SQL_ID) + Constants.COLOR_SANDYORANGE + " )");
            NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_SANDYORANGE + "[Bank: " + Constants.COLOR_DOLLARGREEN + "$" + player.GetSharedData(EntityData.PLAYER_BANK) + Constants.COLOR_SANDYORANGE + "] [Cash: " + Constants.COLOR_DOLLARGREEN + "$" + player.GetSharedData(EntityData.PLAYER_MONEY) + Constants.COLOR_SANDYORANGE + "] [Paycheck: " + Constants.COLOR_DOLLARGREEN + "$" + " TBD" + Constants.COLOR_SANDYORANGE + "]");
            NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_SANDYORANGE + "[Level: " + Constants.COLOR_WHITE + player.GetData(EntityData.PLAYER_LEVEL) + Constants.COLOR_SANDYORANGE + "] [Respect Points: " + Constants.COLOR_WHITE + rp + "/" + maxPoints + Constants.COLOR_SANDYORANGE + "] [" + "Job: " + Constants.COLOR_WHITE + player.GetData(EntityData.PLAYER_JOB) + Constants.COLOR_SANDYORANGE + "] " + Constants.COLOR_SANDYORANGE + "[Contact Number: " + Constants.COLOR_WHITE + "TBD" + Constants.COLOR_SANDYORANGE + "]");
            NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_SANDYORANGE + "[Houses: " + Constants.COLOR_WHITE + player.GetData(EntityData.PLAYER_HOUSES) + "/3" + Constants.COLOR_SANDYORANGE + "] [Vehicles: " + Constants.COLOR_WHITE + player.GetData(EntityData.PLAYER_VEHICLES) + "/3" + Constants.COLOR_SANDYORANGE + "] [Businesses: " + Constants.COLOR_WHITE + player.GetData(EntityData.PLAYER_BUSINESSES) + "/1" + Constants.COLOR_SANDYORANGE + "] [Parking Spaces: " + Constants.COLOR_WHITE + "0/2" + Constants.COLOR_SANDYORANGE + "]");

            if (rp == maxPoints || rp > maxPoints)
            {
                NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_DOLLARGREEN + "You are ready to level up!");
                NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_OCEANBLUE + "Level Upgrade Cost: " + Constants.COLOR_DOLLARGREEN + "$500 " + Constants.COLOR_OCEANBLUE + "/buylevel");
        ...
        [Command(Commands.SHOUT, Alias = "s", Description = Commands.HLP_YELL_COMMAND, GreedyArg = true)]
        public void GritarCommand(Client player, string message)
        {
            ...
            {
                SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_SHOUT, 45.0f);
            }
        }
        ...
        public void SusurrarCommand(Client player, string message)
        {
            ...
            {
                SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_WHISPER, 3.0f);
            }
        }
     ...
     /*   [Command("alias", GreedyArg = true)]
        ...
        */
        ...
        {
            string timeString = "[" + DateTime.Now.ToString("HH:mm");
            SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_ME, player.Dimension > 0 ? 7.5f : 20.0f);
        }
        ...
        {
            string timeString = "[" + DateTime.Now.ToString("HH:mm");
            SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_DO, player.Dimension > 0 ? 7.5f : 20.0f);
        }
        [Command(Commands.COM_OOC, Commands.HLP_OOC_COMMAND, GreedyArg = true)]
        public void OocCommand(Client player, string message)
        {
            string timeString = "[" + DateTime.Now.ToString("HH:mm");
            SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_OOC, player.Dimension > 0 ? 7.5f : 20.0f);
        }
        ...
        public void SuCommand(Client player)
        {
            ...
            else
            {
                ...
                SendMessageToNearbyPlayers(timeString, player, string.Empty, messageType, 20.0f);
            }
        }
        ...
        public void MegafonoCommand(Client player, string message)
        {
            ...
            SendMessageToNearbyPlayers(timeString, player, message, Constants.MESSAGE_MEGAPHONE, 45.0f);
        }

        [Command(Commands.COM_PM, Commands.HLP_PM_COMMAND, GreedyArg = true)]
        public void MpCommand(Client player, string arguments)
        {
            ...
            if (int.TryParse(args[0], out int targetId) == true)
            {
                ...
                if (args.Length < 1)
                {
                    player.SendChatMessage(Constants.COLOR_HELP + Commands.HLP_PM_COMMAND);
                    return;
                }
            }
