    ...
    public class Job : Script
    {
        ...
        public Job()
        {
            foreach (JobPickModel job in Constants.JOB_PICK_LIST)
            {
                ...
                NAPI.TextLabel.CreateTextLabel("Press E to open UI", job.position, 10.0f, 0.5f, 4, new Color(190, 235, 100), false, 0);
        ...
        public static bool IsPlayerOnWorkPlace(Client player)
        {
            ...
            if (job > 0)
            {
                ...
                Vector3 MechanicJob = null;

                switch (job)
                {
                    case Constants.JOB_FASTFOOD:
                        MechanicJob = new Vector3(177.1226f, -1437.733f, 29.24163f);
                        break;
                    case Constants.JOB_HOOKER:
                        Vector3 Paleto = new Vector3(105.7209f, -1302.901f, 28.7688f);  //Vanila Unicorn
                        Vector3 Sandy = new Vector3(2467.066f, 4100.877f, 37.58295f);

                        // Get the closest lockers
                        MechanicJob = player.Position.DistanceTo(Paleto) < player.Position.DistanceTo(Sandy) ? Paleto : Sandy;
                        break;
                    case Constants.JOB_TRUCKER:
                        MechanicJob = new Vector3(-44.04705f, -2519.901f, 7.394626f);

                        break;
                    case Constants.JOB_OCEANCLEANER:
                        MechanicJob = new Vector3(288.0206f, -2981.255f, 5.862742f);

                        break;
                    case Constants.JOB_POSTALGQ:
                        MechanicJob = new Vector3(499.7191f, -651.9603f, 24.90868f);

                        break;
                }
                onWorkPlace = MechanicJob != null && player.Position.DistanceTo(MechanicJob) < 5.0f;
            }
           /*     if (job > 0)
            {
            ...
            else if (faction > 0)
            {
                ...
                switch (faction)
                {
                    ...
                    case Constants.FACTION_DOC:
                        lockers = new Vector3(1682.022f, 2574.967f, 45.91152f);
                        break;
                    case Constants.FACTION_LSC:
                        lockers = new Vector3(-354.4169f, -128.2691f, 39.43061f);
                        break;
        ...
        public void DutyCommand(Client player)
        {
            ...
            else if (player.GetData(EntityData.PLAYER_ON_DUTY) == 1)
            {
                ...
                if (player.GetData(EntityData.PLAYER_FACTION) == 7)
                {
                    player.RemoveAllWeapons();
                }
            }
        ...
        public void OrdersCommand(Client player)
        {
            ...
            else
            {
                ...
                if (player.GetData(EntityData.PLAYER_JOB) == Constants.JOB_POSTALGQ)
                {
                    // Get the fastfood deliverer orders
                    PostalGQ.CheckFastfoodOrders(player);
                    return;
                }
                if (player.GetData(EntityData.PLAYER_JOB) == Constants.JOB_TRUCKER)
                {
                    // Get the fastfood deliverer orders
                    Trucker.CheckTruckingOrders(player);
                    return;
                }
