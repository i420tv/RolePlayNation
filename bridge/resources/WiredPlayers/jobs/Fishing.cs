...
namespace WiredPlayers.jobs
{
    public class Fishing : Script
    {
        ...
        private static void OnFishingPrewarnTimer(object playerObject)
        {
            NAPI.Task.Run(() =>
            {
                ...
                Timer fishingfishTimer = new Timer(FishingSuccessEvent, player, 15500, Timeout.Infinite);
                fishingTimerList.Add(player.Value, fishingfishTimer);
        ...
        private static int GetPlayerFishingLevel(Client player)
        {
            // Get player points
        ...
        public static void FishingSuccessEvent(object playerObject)
        {
            Client player = (Client)playerObject;
            ...
            if (fishingTimerList.TryGetValue(player.Value, out Timer fishingfishTimer) == true)
            {
                fishingfishTimer.Dispose();
                fishingTimerList.Remove(player.Value);
            }
            if (!failed)
            {
        ...
        public void FishCommand(Client player)
        {
            ...
            ItemModel fishingRod = Globals.GetPlayerItemModelFromHash(playerId, Constants.ITEM_HASH_FISHING_ROD);
            if (fishingRod != null)
            {
                //  int playerId = player.GetData(EntityData.PLAYER_SQL_ID);
