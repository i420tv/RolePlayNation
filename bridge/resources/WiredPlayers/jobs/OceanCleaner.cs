...
namespace WiredPlayers.jobs
{
    public class OceanCleaner : Script
    {
        ...
        public static List<OceanItemModel> allOceanItems = new List<OceanItemModel>();
        public static Dictionary<int, Timer> blipTimerList;
        ...
        public OceanCleaner()
        {
            CreateJobLocation();
            CreateNPC();
            blipTimerList = new Dictionary<int, Timer>();
        ...
        [Command("scan")]
        public void Command_Scan(Client player)
        {
            int playerJob = player.GetData(EntityData.PLAYER_JOB);
            if (playerJob == Constants.JOB_OCEANCLEANER)
            {
                foreach (OceanItemModel oceanItem in allOceanItems)
                {
                    if (oceanItem.itemObject != null)
                    {
                        if (player.Position.DistanceTo(oceanItem.itemPosition) < 600f)
                        {
                            oceanItem.blip = NAPI.Blip.CreateBlip(25, oceanItem.itemPosition, 1, 1, "Scrap", 255, 0, true);
                        }
            ...
            {
                player.SendChatMessage(Constants.COLOR_ERROR + "You dont work at the Ocean Cleaning Department");
            }
        }
        //public void scantimer(Client player)
        //{
        //    // Create the timer
        //    Timer miningTimer = null;
        //    // Create the mining timer
        //    miningTimer = new Timer(RemoveBlips, player, 20000, Timeout.Infinite);
        //    // Add the timer to the list
        //    blipTimerList.Add(player.Value, miningTimer);
        //}
        //public static void RemoveBlips(object timer)
        //{
        //    Client player = (Client)timer;

        //    foreach (OceanItemModel oceanItem in allOceanItems)
        //    {
        //        //oceanItem.blip.Delete();
        //    }

        //    // Remove the timer from the list
        //    blipTimerList.Remove(player.Value);
        //}
        //public static void ActuallyRemoveBlips(Client player)
        //{
              ...
              /*  foreach (VehicleModel v in vehicles)
                {
                    ...
                    if (veh.GetData(EntityData.VEHICLE_ID) == null)
                    {
                        return;                                                     
                    }
                } */
        ...
        public static void Command_Job_OceanCleaner_Collect(Client player)
        {
            foreach (OceanItemModel oceanItem in allOceanItems)
            {
                if (oceanItem.itemPosition.DistanceTo(player.Position) < 10)
                {
                   ...
                   // player.SendChatMessage("You found " + oceanItem.itemName);

                    oceanItem.itemText.Delete();
                    oceanItem.itemObject.Delete();
                    if(oceanItem.blip != null)
                    {
                        oceanItem.blip.Delete();
                    }
