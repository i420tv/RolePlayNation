            ...
            else
            {
                ...
                switch (parking.type)
                {
                    case Constants.PARKING_TYPE_DEPOSIT:
                        ...
                        if (playerMoney >= vehicle.price / 100)
                        {
                            
                            player.SetSharedData(EntityData.PLAYER_MONEY, playerMoney - vehicle.price / 100);

                            string message = string.Format(InfoRes.unpark_money, vehicle.price / 100);
                            player.SendChatMessage(Constants.COLOR_INFO + message);
                        }
                        else
                        {
                            string message = string.Format(ErrRes.parking_not_money, vehicle.price / 100);
